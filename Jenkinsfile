pipeline {
    ...
    stages {
        ...
        stage('Build Docker Image') {
            ...
            steps {
                script {
                    app = docker.build("willbla/train-schedule")    // uses docker plugin to build image with name mentioned in ""
                    app.inside {
                        sh 'echo $(curl localhost:8080)'    //to test the image if built successfully by executing the command inside image
                    }
                }
            }
        }
            ...
            steps {
                script {
                    docker.withRegistry('https://registry.hub.docker.com', 'docker_hub_login') {
                        app.push("${env.BUILD_NUMBER}")    //to push image with version
                        app.push("latest")
                    }
                }
            }
